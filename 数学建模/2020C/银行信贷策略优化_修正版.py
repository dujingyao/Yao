...
class BankCreditOptimizer:
    ...
    def optimize_credit_strategy(self, total_budget=10000):
        ...
        print(f"   贷款额度范围: {self.最小贷款额度}-{self.最大贷款额度}万元")
    ...
    def _make_bank_credit_decision(self, 信誉评级, 历史违约, 风险评分, enterprise_data):
        ...
        if 信誉评级 == 'A':
            if not 历史违约:
                基础额度 = min(self.最大贷款额度, max(self.最小贷款额度, 年营业收入 * 0.25))
            ...
            if not 历史违约:
                基础额度 = min(80, max(self.最小贷款额度, 年营业收入 * 0.20))
                基础利率 = 0.055 + 0.03 * 风险评分
                决策依据 = "B级标准客户"
            else:
                基础额度 = min(60, max(self.最小贷款额度, 年营业收入 * 0.12))
                基础利率 = 0.070 + 0.04 * 风险评分
                决策依据 = "B级违约客户降额提率"
                
        elif 信誉评级 == 'C':
            if not 历史违约:
                基础额度 = min(50, max(self.最小贷款额度, 年营业收入 * 0.10))
                基础利率 = 0.080 + 0.05 * 风险评分
                决策依据 = "C级客户高风险高利率"
            else:
                基础额度 = min(30, max(self.最小贷款额度, 年营业收入 * 0.08))
        ...
        # 应用银行约束
        最终额度 = max(0, min(self.最大贷款额度, 基础额度)) if 基础额度 >= self.最小贷款额度 else 0
            ...
            for idx, row in 获贷企业.iterrows():
                if cumulative_amount + row['贷款额度(万元)'] <= total_budget:
                    ...
                    if remaining >= self.最小贷款额度:
                        self.信贷策略.loc[idx, '贷款额度(万元)'] = min(remaining, self.最大贷款额度)
    ...
    def generate_bank_report(self):
        ...
        print(f"\n📊 六、贷款额度分布")
        if len(获贷企业) > 0:
            额度区间 = [(10, 30), (30, 50), (50, 80), (80, 100)]
            区间名称 = ["10-30万", "30-50万", "50-80万", "80-100万"]
    ...
    def export_results(self):
        ...
        try:
            # 导出完整策略
            self.信贷策略.to_excel('银行信贷策略_10到100万额度.xlsx', index=False)
            print(f"💾 完整信贷策略已保存: 银行信贷策略_10到100万额度.xlsx")
            
            # 导出获贷企业
            获贷企业 = self.信贷策略[self.信贷策略['贷款额度(万元)'] >= self.最小贷款额度]
            获贷企业.to_excel('获贷企业名单_10到100万.xlsx', index=False)
            print(f"💾 获贷企业名单已保存: 获贷企业名单_10到100万.xlsx")
            
            # 导出拒贷企业
            拒贷企业 = self.信贷策略[self.信贷策略['贷款额度(万元)'] < self.最小贷款额度]
            拒贷企业.to_excel('拒贷企业名单_10到100万.xlsx', index=False)
            print(f"💾 拒贷企业名单已保存: 拒贷企业名单_10到100万.xlsx")
...
def main():
    """主函数"""
    print("🏦 银行信贷策略优化系统")
    print("贷款额度：10-100万元，年利率：4%-15%，期限：1年")
    print("=" * 80)
