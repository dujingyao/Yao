...
def plot_image_comparison(image1, image2, save_path=None):
    ...
    try:
        axes[0].set_title("BT2020源图像", fontsize=14)
    except:
    ...
    try:
        axes[1].set_title("映射后显示图像", fontsize=14)
    except:
...
def plot_delta_e_curve(delta_e_values, save_path=None):
    ...
    plt.plot(delta_e_values, 'b-', linewidth=2, label=f"ΔE值 (均值: {mean_delta_e:.3f})")
    plt.axhline(y=mean_delta_e, color='r', linestyle='--', alpha=0.7, label=f"平均值: {mean_delta_e:.3f}")
    plt.axhline(y=max_delta_e, color='orange', linestyle='--', alpha=0.7, label=f"最大值: {max_delta_e:.3f}")
    
    try:
        plt.title("颜色转换ΔE误差分布", fontsize=16)
        plt.xlabel("像素索引", fontsize=12)
        plt.ylabel("ΔE色差", fontsize=12)
        stats_text = f"总像素数: {len(delta_e_values)}\n平均ΔE: {mean_delta_e:.3f}\n最大ΔE: {max_delta_e:.3f}\n标准差: {np.std(delta_e_values):.3f}"
    except:
...
def plot_cie1931_color_space(converter, source_points=None, mapped_points=None, save_path=None):
    ...
    ax.plot(spectral_x, spectral_y, 'k-', linewidth=2, label='CIE1931光谱轨迹')
    ...
    if source_points is not None and mapped_points is not None:
        try:
            ...
            source_label = 'Source Colors'
            mapped_label = 'Mapped Colors'
            
        ax.scatter(source_points[:, 0], source_points[:, 1], 
                  c='blue', s=30, alpha=0.6, label=source_label)
        ax.scatter(mapped_points[:, 0], mapped_points[:, 1], 
                  c='red', s=30, alpha=0.6, label=mapped_label)
    ...
    try:
        ax.set_title('CIE1931色度图与颜色空间转换', fontsize=16)
    except:
