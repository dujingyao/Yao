...
class PracticalColorConverter:
    ...
    def find_gamut_intersection(self, center, direction):
        ...
        for i in range(3):
            ...
            cross_prod = np.cross(direction, edge_vec)
            if abs(cross_prod) < 1e-10:
                continue  # 平行
            
            t = np.cross(center_to_p1, edge_vec) / cross_prod
            u = np.cross(center_to_p1, direction) / cross_prod
...
def create_comprehensive_plots(original_image, mapped_image, converter, 
    ...
    ax4.set_title("ΔE Color Difference Map", fontsize=14, fontweight='bold')
    ax4.axis('off')
    cbar2 = plt.colorbar(im2, ax=ax4, shrink=0.8)
    cbar2.set_label('ΔE Value')
    ...
    ax7.set_xlabel('ΔE Value', fontsize=12)
    ax7.set_ylabel('Density', fontsize=12)
    ax7.set_title('ΔE Distribution Histogram', fontsize=14, fontweight='bold')
    ...
    stats_data = [
        ['Metric', 'Value'],
        ['Mean ΔE', f'{np.mean(delta_e_values):.3f}'],
        ['Median ΔE', f'{np.median(delta_e_values):.3f}'],
        ['Std ΔE', f'{np.std(delta_e_values):.3f}'],
        ['Max ΔE', f'{np.max(delta_e_values):.3f}'],
        ['Min ΔE', f'{np.min(delta_e_values):.3f}'],
    ...
    1. 色域检测: 使用CIE 1931 xy色度图检测颜色是否在目标显示器色域内
    2. 智能压缩: 对超出色域的颜色，采用基于饱和度和亮度的自适应压缩策略
    3. 边界投影: 计算从色域中心到边界的最优投影点，保持色彩的感知一致性
    4. 色差评估: 使用CIE ΔE*ab公式量化转换前后的色彩差异
    
    质量标准:
    • 优秀 (ΔE < 1): 人眼几乎无法察觉的差异
    • 良好 (1 ≤ ΔE < 3): 训练有素的观察者可能察觉的轻微差异  
    • 可接受 (3 ≤ ΔE < 6): 一般观察者可察觉但可接受的差异
    • 较差 (6 ≤ ΔE < 10): 明显的色彩差异，需要优化
    • 很差 (ΔE ≥ 10): 严重的色彩失真，不可接受
    """
    
    ax9.text(0.02, 0.98, explanation_text, transform=ax9.transAxes, fontsize=11,
            verticalalignment='top', bbox=dict(boxstyle="round,pad=0.5", facecolor="lightblue", alpha=0.8))
    
    plt.suptitle('LED显示屏颜色转换设计与校正 - 问题1 实用算法完整分析', 
                fontsize=18, fontweight='bold', y=0.98)
    
    plt.savefig(os.path.join(save_dir, 'practical_complete_analysis.png'), 
               dpi=300, bbox_inches='tight')
    plt.close()

def main():
    """主函数"""
    print("=== LED显示屏颜色转换设计与校正 - 问题1 (实用版本) ===")
    
    # 创建输出目录
    output_dir = "practical_results"
    ...
    np.savez(os.path.join(output_dir, 'practical_conversion_results.npz'), **practical_results)
    
    # 创建详细报告
    with open(os.path.join(output_dir, 'practical_detailed_report.txt'), 'w', encoding='utf-8') as f:
        f.write("LED显示屏颜色转换设计与校正 - 问题1 实用算法详细报告\n")
        f.write("=" * 80 + "\n\n")
        
        f.write("1. 算法概述\n")
        f.write("-" * 30 + "\n")
        f.write("本实用算法采用了以下核心技术:\n")
        f.write("• 精确的CIE 1931色度坐标转换\n")
        f.write("• 基于几何的色域边界检测\n")
        f.write("• 自适应的智能压缩映射策略\n")
        f.write("• 感知一致性的色域投影算法\n")
        f.write("• 标准CIE ΔE*ab色差评估\n\n")
        ...
        f.write(f"95百分位数: {np.percentile(delta_e_values, 95):.4f}\n")
        f.write(f"99百分位数: {np.percentile(delta_e_values, 99):.4f}\n\n")
        ...
        for (low, high), label in zip(ranges, range_labels):
            ...
            count = np.sum(mask)
            percentage = count / len(delta_e_values) * 100
            f.write(f"{label}: {count:6} 像素 ({percentage:5.1f}%)\n")
        f.write(f"总像素数: {len(delta_e_values):,}\n\n")
        
        f.write("4. 算法特点与优势\n")
        f.write("-" * 30 + "\n")
        f.write("• 实用性强: 算法运行稳定，适合实际工程应用\n")
        f.write("• 感知优化: 保持色彩的视觉一致性，减少明显失真\n")
        f.write("• 自适应性: 根据颜色特征调整映射策略\n")
        f.write("• 标准兼容: 使用国际标准的色差评估方法\n")
        f.write("• 高效计算: 算法复杂度适中，便于实时处理\n\n")
        
        f.write("5. 应用建议\n")
        f.write("-" * 30 + "\n")
        f.write("• 对于要求较高的应用，建议进一步优化高饱和度颜色的处理\n")
        f.write("• 可考虑结合硬件校准来提升整体显示质量\n")
        f.write("• 针对特定内容类型可以调整压缩参数\n")
        f.write("• 建议定期使用标准测试图像验证转换效果\n")
    
    print(f"\n=== 实用算法分析完成 ===")
    print(f"详细结果已保存到: {output_dir}/")
    print("生成的文件:")
    print("1. practical_complete_analysis.png - 完整分析图表")
    print("2. practical_conversion_results.npz - 数值结果")
    print("3. practical_detailed_report.txt - 详细分析报告")
